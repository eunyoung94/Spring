...
/*
 ...
 [jdbc자체]
 [Hibernate]
 ORM (Object Relation Mapping) 자바객체와 관계현 db와 매핑 
 */
...
public class JdbcNoticeDAO implements NoticeDAO {
	...
	public List selectAll() {
		...
		// 매개변수 순서 : 쿼리문 바인트변수를 배열로 처리, 매핑객체(ResultSet의 데이터를 담는객체)
	...
	// jdbcTemplate에서는 DML은 update로 메서드처리
	@Override
	public void insert(Notice notice) throws DMLException {
		...
		if (result == 0) {
			throw new DMLException("등록에 실패했습니다");
		}
	}
	...
	public void delete(int notice_id) throws DMLException {
		...
		if (result == 0) {
			throw new DMLException("삭제에 실패했서요");
		}
